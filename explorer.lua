...
local target_distance_states = {150, 60, 30, 5}
...
local function find_unstuck_target(center, radius)
    console.print("Finding unstuck target.")
    local player_pos = center or get_player_position()
    local search_radius = radius or unstuck_target_distance
    local valid_targets = {}
    ...
    for x = -search_radius, search_radius, grid_size do
        for y = -search_radius, search_radius, grid_size do
            ...
            if utility.is_point_walkeable(point) and distance >= 2 and distance <= search_radius then
...
local failed_target_attempts = 0
local max_failed_attempts = 5  -- Maximale Anzahl von Versuchen, bevor ein näheres Ziel gewählt wird
...
local function check_if_stuck()
    ...
    if last_position and calculate_distance(current_pos, last_position) < 1 then
        if current_time - last_move_time > stuck_threshold then
            if target_position then
                local distance_to_target = calculate_distance(current_pos, target_position)
                if distance_to_target > max_target_distance * 0.8 then  -- Wenn die Distanz zum Ziel zu groß ist
                    failed_target_attempts = failed_target_attempts + 1
                    if failed_target_attempts >= max_failed_attempts then
                        console.print("Mehrere Versuche fehlgeschlagen. Wähle ein näheres Ziel.")
                        return "new_closer_target"
                    end
                end
            end
            return true
        ...
        failed_target_attempts = 0  -- Zurücksetzen der fehlgeschlagenen Versuche, wenn Bewegung erkannt wird
    end
...
on_update(function()
    ...
    local current_core_time = get_time_since_inject()
    if current_core_time - last_call_time > 10 then
        ...
        check_walkable_area()
        local stuck_status = check_if_stuck()
        if stuck_status == true then
        ...
        elseif stuck_status == "new_closer_target" then
            console.print("Multiple attempts failed. Selecting a closer target.")
            local player_pos = get_player_position()
            target_position = find_target(true, player_pos, max_target_distance * 0.5)  -- Suche ein näheres Ziel
            target_position = set_height_of_valid_position(target_position)
            last_move_time = os.time()
            current_path = {}
            path_index = 1
            failed_target_attempts = 0  -- Zurücksetzen der fehlgeschlagenen Versuche
        end
    end
